...
class cprodutos_edit extends cprodutos {
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "")
			$this->Page_Terminate("produtoslist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// produto
			$this->produto->SetDbValueDef($rsnew, $this->produto->CurrentValue, NULL, $this->produto->ReadOnly);
...
fprodutosedit.ValidateRequired = true;
<?php } else { ?>
fprodutosedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fprodutosedit.Lists["x_Categoria"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_categoia","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<div>
<?php if ($produtos->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_produtos_id" class="col-sm-2 control-label ewLabel"><?php echo $produtos->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $produtos->id->CellAttributes() ?>>
<span id="el_produtos_id">
<span<?php echo $produtos->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $produtos->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="produtos" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($produtos->id->CurrentValue) ?>">
<?php echo $produtos->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
