	...
	function Page_Main() {
		...
		if ($Security->CurrentUserID() <> "" && !$Security->IsAdmin()) { // Non system admin
			if ($this->getCurrentMasterTable() == "entregas")
				$this->DbMasterFilter = $this->AddMasterUserIDFilter($this->DbMasterFilter, "entregas"); // Add master User ID filter
			if ($this->getCurrentMasterTable() == "endereco")
				$this->DbMasterFilter = $this->AddMasterUserIDFilter($this->DbMasterFilter, "endereco"); // Add master User ID filter
		}
		ew_AddFilter($sFilter, $this->DbDetailFilter);
		ew_AddFilter($sFilter, $this->SearchWhere);

		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "entregas") {
			global $entregas;
			$rsmaster = $entregas->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("entregaslist.php"); // Return to master page
			} else {
				$entregas->LoadListRowValues($rsmaster);
				$entregas->RowType = EW_ROWTYPE_MASTER; // Master row
				$entregas->RenderListRow();
				$rsmaster->Close();
			}
		}

		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "endereco") {
			global $endereco;
			$rsmaster = $endereco->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("enderecolist.php"); // Return to master page
			} else {
				$endereco->LoadListRowValues($rsmaster);
				$endereco->RowType = EW_ROWTYPE_MASTER; // Master row
				$endereco->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetall") {
				...
				$this->DbMasterFilter = "";
				$this->DbDetailFilter = "";
				$this->id_endereco->setSessionValue("");
				$this->id_endereco->setSessionValue("");
			}
...
fpedidolist.Lists["x_id_produto"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_produto","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php if (($pedido->Export == "") || (EW_EXPORT_MASTER_RECORD && $pedido->Export == "print")) { ?>
<?php
$gsMasterReturnUrl = "entregaslist.php";
if ($pedido_list->DbMasterFilter <> "" && $pedido->getCurrentMasterTable() == "entregas") {
	if ($pedido_list->MasterRecordExists) {
		if ($pedido->getCurrentMasterTable() == $pedido->TableVar) $gsMasterReturnUrl .= "?" . EW_TABLE_SHOW_MASTER . "=";
?>
<?php include_once "entregasmaster.php" ?>
<?php
	}
}
?>
<?php
$gsMasterReturnUrl = "enderecolist.php";
if ($pedido_list->DbMasterFilter <> "" && $pedido->getCurrentMasterTable() == "endereco") {
	if ($pedido_list->MasterRecordExists) {
		if ($pedido->getCurrentMasterTable() == $pedido->TableVar) $gsMasterReturnUrl .= "?" . EW_TABLE_SHOW_MASTER . "=";
?>
<?php include_once "enderecomaster.php" ?>
<?php
	}
}
?>
...
<?php
	...
	if ($pedido->CurrentAction == "" && $pedido_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$pedido_list->setWarningMessage($Language->Phrase("NoPermission"));
...
<form name="fpedidolist" id="fpedidolist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<table id="tbl_pedidolist" class="table ewTable">
...
// Header row
$pedido_list->RowType = EW_ROWTYPE_HEADER;
...
<?php if ($pedido->id_produto->Visible) { // id_produto ?>
	...
	<?php } else { ?>
		<th data-name="id_produto"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pedido->SortUrl($pedido->id_produto) ?>',1);"><div id="elh_pedido_id_produto" class="pedido_id_produto">
...
<span id="el<?php echo $pedido_list->RowCnt ?>_pedido_id_produto" class="pedido_id_produto">
<span<?php echo $pedido->id_produto->ViewAttributes() ?>>
<?php echo $pedido->id_produto->ListViewValue() ?></span>
</span>
<a id="<?php echo $pedido_list->PageObjName . "_row_" . $pedido_list->RowCnt ?>"></a></td>
	<?php } ?>
<?php
