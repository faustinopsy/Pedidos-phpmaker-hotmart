...
//
// Table class for entregas
//
class centregas extends cTable {
	var $id;
	var $endereco;
	var $utmx;
	var $utmy;
	...
	function __construct() {
		...
		$this->TableVar = 'entregas';
		$this->TableName = 'entregas';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`entregas`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('entregas', 'entregas', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// endereco
		$this->endereco = new cField('entregas', 'entregas', 'x_endereco', 'endereco', '`endereco`', '`endereco`', 200, -1, FALSE, '`endereco`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['endereco'] = &$this->endereco;

		// utmx
		$this->utmx = new cField('entregas', 'entregas', 'x_utmx', 'utmx', '`utmx`', '`utmx`', 200, -1, FALSE, '`utmx`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['utmx'] = &$this->utmx;

		// utmy
		$this->utmy = new cField('entregas', 'entregas', 'x_utmy', 'utmy', '`utmy`', '`utmy`', 200, -1, FALSE, '`utmy`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['utmy'] = &$this->utmy;

		// icon
		$this->icon = new cField('entregas', 'entregas', 'x_icon', 'icon', '`icon`', '`icon`', 200, -1, FALSE, '`icon`', FALSE, FALSE, FALSE, 'IMAGE', 'TEXT');
		$this->icon->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['icon'] = &$this->icon;

		// status
		$this->status = new cField('entregas', 'entregas', 'x_status', 'status', '`status`', '`status`', 3, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
		$this->status->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['status'] = &$this->status;

		// NOME
		$this->NOME = new cField('entregas', 'entregas', 'x_NOME', 'NOME', '`NOME`', '`NOME`', 200, -1, FALSE, '`NOME`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['NOME'] = &$this->NOME;

		// TELEFONE
		$this->TELEFONE = new cField('entregas', 'entregas', 'x_TELEFONE', 'TELEFONE', '`TELEFONE`', '`TELEFONE`', 200, -1, FALSE, '`TELEFONE`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function GetDetailUrl() {
		...
		if ($this->getCurrentDetailTable() == "pedido") {
			$sDetailUrl = $GLOBALS["pedido"]->GetListUrl() . "?" . EW_TABLE_SHOW_MASTER . "=" . $this->TableVar;
			$sDetailUrl .= "&fk_id=" . urlencode($this->id->CurrentValue);
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "entregaslist.php";
		}
		return $sDetailUrl;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`entregas`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("entregasview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("entregasview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "entregasadd.php?" . $this->UrlParm($parm);
		else
			$url = "entregasadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("entregasedit.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("entregasedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("entregasadd.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("entregasadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("entregasdelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->utmx->EditValue = $this->utmx->CurrentValue;
		...
		$this->utmy->EditValue = $this->utmy->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->utmx->Exportable) $Doc->ExportCaption($this->utmx);
					if ($this->utmy->Exportable) $Doc->ExportCaption($this->utmy);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->utmx->Exportable) $Doc->ExportField($this->utmx);
						if ($this->utmy->Exportable) $Doc->ExportField($this->utmy);
